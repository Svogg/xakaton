...
async def add_city(city: CityModel, current_session: AsyncSession = Depends(get_async_session)):
    async with open('17_dataset/cities.json', 'r', encoding='utf-8') as cities:
        ...
        for i in data_cities:
            try:
                stmt = insert(city).values(CitySchema(
...
async def add_event(event: EventModel, current_session: get_async_session):
    async with open('17_dataset/events.json', 'r', encoding='utf-8') as events:
            ...
            try:
                if len(i.get('dictionary_data').get('schedule')) != 0:
                    if isinstance(i.get('dictionary_data').get('schedule')[0].get('start'), dict):
                        stmt = insert(event).values(EventSchema(
                            id=i.get('_id').get('$oid'),
...
async def add_excursion(excursion: ExcursionModel, current_session: get_async_session):
    async with open('17_dataset/excursions.json', 'r', encoding='utf-8') as excursions:
        ...
        for i in data_excursions:
            try:
                if isinstance(i.get('dictionary_data').get('season_start'), str):
                    stmt = insert(excursion).values(ExcursionSchema(
                        id=i.get('_id').get('$oid'),
                        city_id=i.get('dictionary_data').get('city'),
...
async def add_restaurant(restaurant: RestaurantModel, current_session: AsyncSession):
    async with open('17_dataset/restaurants.json', 'r', encoding='utf-8') as restaurants:
        data_restaurants = await json.load(restaurants)
        for i in data_restaurants:
            try:
                if i.get('dictionary_data').get('geo_data') is not None:
                    stmt = insert(restaurant).values(RestaurantSchema(
                        id=i.get('_id').get('$oid'),
